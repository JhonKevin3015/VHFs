function [dens,theta,uinit,etainit,zc,dh,Us,Ri,Ls,Lsmin,ts] = frontInitCondi(ny,nx,nz,deltaxy,perturba)
...
ts = sqrt(54/5)*sqrt(1+Ri)/f0/3600; % in hours
...
%dirsave='/Users/oceano/MIT-JOB-TESIS/CI-MITrunmodel/'
%prec='float32'     % Para el caso de EXP1
        % pared en las paredes
%        hh(:,1:2)=0
%        hh(:,end-1:end)=0
%fid=fopen([dirsave 'INI_topo_sl.bin'],'w',ieee); fwrite(fid,hh,prec); fclose(fid);
...
switch IC
    ...
    case 'T08' 
        ...
        for i=1:nx
            for j=1:ny
                uinit(i,j,:) = flipud(cumtrapz(fliplr(zc), flipud(squeeze(g*a./f0*deltatheta*ystrucyfull(i,j,:)))));
            end
        end
